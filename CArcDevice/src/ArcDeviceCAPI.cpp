...
// | Globals
// +------------------------------------------------------------------------------------+
static std::unique_ptr<arc::gen3::CArcDevice> g_pDevice;

static char		g_szErrMsg[ ARC_ERROR_MSG_SIZE ];
static char		g_szTmpBuf[ ARC_MSG_SIZE ];

static char**	g_pDeviceStringList = nullptr;
...
// +------------------------------------------------------------------------------------+
#define VERIFY_CLASS_PTR( uiDeviceNumber )												\
...
// |
// |  <IN>  -> uiDeviceNumber : The device (PCI(e)) to access
...
GEN3_CARCDEVICE_API const char* ArcDevice_ToString( unsigned int uiDeviceNumber, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( uiDeviceNumber )

		ArcSprintf( g_szTmpBuf, ARC_MSG_SIZE, "%s", g_pDevice->toString().c_str() );
	}
	...
	*pStatus = ARC_STATUS_OK;
	...
	if ( ArcDevice_DeviceCount() <= 0 )
	{
		*pStatus = ARC_STATUS_ERROR;
		...
		ArcSprintf( g_szErrMsg, ARC_ERROR_MSG_SIZE, "%s", "No ARC, Inc. PCI or PCIe devices found!\n" );
	}
}
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		if ( ArcDevice_DeviceCount() > 0 )
		{
			...
			if ( g_pDeviceStringList != nullptr )
			{
				ArcDevice_FreeDeviceStringList();
			}

			g_pDeviceStringList = new char* [ ArcDevice_DeviceCount() ];
			...
			{
				g_pDeviceStringList[ i ] = new char[ 100 ];

				ArcSprintf( &g_pDeviceStringList[ i ][ 0 ], 100, "%s", sPCIList[ i ].c_str() );
			}
			...
			for ( std::uint32_t i = arc::gen3::CArcPCI::deviceCount(); i < ArcDevice_DeviceCount(); i++ )
			{
				...
				ArcSprintf( &g_pDeviceStringList[ i ][ 0 ], 100, "%s", sPCIeList[ i - arc::gen3::CArcPCI::deviceCount() ].c_str() );
			}
		}
	}
	catch ( const std::exception& e )
	...
	return const_cast< const char** >( g_pDeviceStringList );
}
...
GEN3_CARCDEVICE_API void ArcDevice_FreeDeviceStringList()
{
	if ( g_pDeviceStringList != nullptr )
	{
		...
		{
			delete[] g_pDeviceStringList[ i ];
		}

		delete[] g_pDeviceStringList;
	}

	g_pDeviceStringList = nullptr;
}
...
GEN3_CARCDEVICE_API unsigned int ArcDevice_IsOpen( int* pStatus )
{
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		if ( g_pDevice != nullptr )
		{
			uiOpen = ( g_pDevice->isOpen() == true ? 1 : 0 );
		}
	}
	catch ( const std::exception& e )
	{
...
GEN3_CARCDEVICE_API void ArcDevice_Open( unsigned int uiDeviceNumber, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		{
			g_pDevice.reset( new arc::gen3::CArcPCIe() );
		}

		else if ( sDevice.find( "PCI" ) != std::string::npos )
		{
			g_pDevice.reset( new arc::gen3::CArcPCI() );
		}
		...
		VERIFY_CLASS_PTR( g_pDevice )

			g_pDevice->open( iDevNum );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->open( iDevNum, uiBytes );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->open( iDevNum, uiRows, uiCols );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->mapCommonBuffer( uiBytes );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->reMapCommonBuffer( uiBytes );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->fillCommonBuffer( uwValue );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			u64PA = g_pDevice->commonBufferPA();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			u64BufSize = g_pDevice->commonBufferSize();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->set2xFOTransmitter( ( uiOnOff == 1 ? true : false ) );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->loadDeviceFile( pszFile );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiReply = g_pDevice->command( { uiBoardId,
												   uiCommand,
												   uiArg1,
												   uiArg2,
												   uiArg3,
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiCtlrId = g_pDevice->getControllerId();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiConnected = ( g_pDevice->isControllerConnected() == true ? 1 : 0 );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->setupController( ( uiReset == 1 ? true : false ),
				( uiTDL == 1 ? true : false ),
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->loadControllerFile( pszFilename, ( uiValidate == 1 ? true : false ), false );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->setImageSize( uiRows, uiCols );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
GEN3_CARCDEVICE_API unsigned int ArcDevice_GetCCParams( int* pStatus )
{
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		uiCCParams = g_pDevice->getCCParams();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
GEN3_CARCDEVICE_API unsigned int ArcDevice_IsCCParamSupported( unsigned int uiParameter, int* pStatus )
{
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		uiSupported = ( g_pDevice->isCCParamSupported( uiParameter ) ? 1 : 0 );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiIsBin = ( g_pDevice->isBinningSet() ? 1 : 0 );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
GEN3_CARCDEVICE_API void ArcDevice_UnSetBinning( unsigned int uiRows, unsigned int uiCols, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		g_pDevice->unSetBinning( uiRows, uiCols );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
												...
												unsigned int uiSubCols, unsigned int uiBiasOffset, unsigned int uiBiasWidth, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		g_pDevice->setSubArray(  *pOldRows,
										*pOldCols,
										uiRow,
										uiCol,
										uiSubRows,
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->unSetSubArray( uiRows, uiCols );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiIsSyn = ( g_pDevice->isSyntheticImageMode() ? 1 : 0 );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
GEN3_CARCDEVICE_API void ArcDevice_SetSyntheticImageMode( unsigned int uiMode, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		g_pDevice->setSyntheticImageMode( ( uiMode == 1 ? true : false ) );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->setOpenShutter( ( uiShouldOpen == 1 ? true : false ) );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
// +----------------------------------------------------------------------------
GEN3_CARCDEVICE_API
void ArcDevice_Expose( float fExpTime, unsigned int uiRows, unsigned int uiCols, void ( *pExposeCall )( float ), void ( *pReadCall )( unsigned int ), unsigned int uiOpenShutter, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		g_pDevice->expose( fExpTime, uiRows, uiCols, g_bAbort, &ExpIFace, ( uiOpenShutter == 1 ? true : false ) );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiIsROUT = ( g_pDevice->isReadout() ? 1 : 0 );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiPixCnt = g_pDevice->getPixelCount();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiPixCnt = g_pDevice->getCRPixelCount();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiFrameCnt = g_pDevice->getFrameCount();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiError = ( g_pDevice->containsError( uiWord ) ? 1 : 0 );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiError = ( g_pDevice->containsError( uiWord, uiWordMin, uiWordMax ) ? 1 : 0 );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			ArcSprintf( g_szTmpBuf,
				ARC_MSG_SIZE,
				"%s",
				g_pDevice->getNextLoggedCmd().c_str() );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			uiCount = g_pDevice->getLoggedCmdCount();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->setLogCmds( ( uiOnOff == 1 ? true : false ) );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			gTemp = g_pDevice->getArrayTemperature();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			gTempDN = g_pDevice->getArrayTemperatureDN();
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->setArrayTemperature( gTempVal );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->loadTemperatureCtrlData( pszFilename );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
	...
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		VERIFY_CLASS_PTR( g_pDevice )
			...
			g_pDevice->saveTemperatureCtrlData( pszFilename );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
//// | Globals
//// +------------------------------------------------------------------------------------+
//static std::unique_ptr<arc::gen3::CArcDevice> g_pDevice( static_cast<arc::gen3::CArcDevice*>( nullptr ) );
//
//static char g_szErrMsg[ ARC_ERROR_MSG_SIZE ];
//static char g_szTmpBuf[ ARC_MSG_SIZE ];
//
